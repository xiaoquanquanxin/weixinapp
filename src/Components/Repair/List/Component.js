...
export default class RepairList extends React.Component {
	...
	render() {
		...
		return <div className={'Components-Repair-List-container'} >
			...
			>
			{tabs.length>0&&<Tabs onChange={(tab, index) => {changeTab(index)}}
					  tabs={tabs} 
					  initialPage={0} 
					  animated={false} 
					  useOnPan={false}>
			...
			<div className={'Repair-list'}>
				{
					repairList.length > 0 && repairList.map((v,i)=>{
						return(
							<div className={"listitem"} key={i} onClick={() => { history.push(`RepairDetails/${v.id}`)}}>
								<div className={"pad"}>
				...
				<div className={"btn"} onClick={() => { history.push('/ComplaintSuggestions?phone=1')}}>
						<img src={phoneimg} />
					</div>
				<div className={"btn"} onClick={() => { history.push('/AddRepair') }}>
						<img src={addimg} />
					</div>
