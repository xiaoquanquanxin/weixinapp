...
export default class Details extends React.Component {
	...
	render () {
		...
		let rstatus = [0, '受理', '接单', '完工', '结束',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ','备注',];

		console.log(111, repairDetaildata.workInfo);
		return <div className={'Components-Repair-Details-container'} >
				...
				<div className={'Components-Repair-Details-container'} >
					<article className="repair-details" >
						<section className={'repair-item relative_div'} >
							...
							{/* {repairDetaildata.imgesUrl && repairDetaildata.imgesUrl.split(" ")} */}
							<div className={'m_imgtop10'} >
								{
									repairDetaildata.imgesUrl &&
									repairDetaildata.imgesUrl.split(',').map((v, i) => {
										return (

											<span key={i} onClick={() => { this.props.onClick(v); }} ><img
												src={v} /></span >
							...
							<div className={`status-flag-g ${stu[repairDetaildata.status]}`} >
								{repairDetaildata.evaluate == 1 ? '已评价' : ste[repairDetaildata.status]}
							</div >
						</section >
					</article >
				</div >
			...
			{/*路由*/}
			{stu[repairDetaildata.status] != 's1' &&
							...
							<Steps current={1} >
								{
									v.routeList && v.routeList.map((vv, ii) => {
										return (
											<Step
												key={ii}
												title={<div className={`routerTitle ${ii == 0 && 'routerTitlewidth'}`} ><span >{vv.describe}
													{
												...
												icon={<span className={'status'}   >{rstatus[vv.workStatus]}</span >}
												description={<div className={'steps'} >
													{stu[repairDetaildata.status] == 's3' && ii == 0 && repairDetaildata.evaluate == 0 &&
													< span className={'evaluate'}
														   onClick={() => { history.push(`/OwnerComment/${this.props.match.params.id}/1`); }} >评价</span >
													}
													{ii == 0 && repairDetaildata.evaluate == 1 &&
													< span className={'evaluate'}
														   onClick={() => { history.push(`/OwnerComment/${this.props.match.params.id}/0`); }} >查看</span >
													}
													...
													<div className={'routeimg'} >
														{
															...
															vv.routeImageUrl && vv.routeImageUrl.split(',').map((vvv, iii) => {
																return (
																	<span onClick={() => { this.props.onClick(vvv); }}
																		  key={iii} >
			...
			{
				stu[repairDetaildata.status] == 's1' && <div className={'Components-Repair-Details-Notice-container'} >
			...
			<WingBlank >
				{stu[repairDetaildata.status] == 's3' && repairDetaildata.evaluate == 0 &&
				<Mybutton callback={() => { history.push(`/OwnerComment/${this.props.match.params.id}/1`);}} type="blue"
						  label="评 价" />
				}
			</WingBlank >
		</div >;
