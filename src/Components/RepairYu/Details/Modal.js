		...
		return (<div>
			...
			>	
				<List renderHeader={() => <div className={'tit'} >评价</div >} className="popup-list" >
								...
								((() => {
									...
									for (let item = 0; item <5; item++) {
										if (item >= this.state.indexStar) {
											array.push(<p key={item} className={'star-box'} onClick={() => this.handClick(parseInt((item+1),10))} ><img src={`${star_gray}`} /></p >);
										} else {
											array.push(<p key={item} className={'star-box'}  onClick={() => this.handClick(parseInt((item+1),10))} ><img src={`${star_yellow}`} /></p >);
										}
									}
									return array;
								})())
					...
					<List.Item >
						<Mybutton callback={this.onClose('modal2')} type="blue" label="匿名提交" />
					</List.Item >
