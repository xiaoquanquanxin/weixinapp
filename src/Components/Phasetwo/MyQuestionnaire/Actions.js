...
class Actions {
        ...
        switch (type) {
            case 1: {
                ...
                if (+result.resultCode === 0) {
                    const {survey, questionList} = result.data;
                    if (survey) {
                        shareSurveyName = survey.surveyName;
                        shareSurveyDesc = survey.surveyDesc;
                    }
                    this.store.questionDetailDta = questionList;
                } else {
    ...
    radiofun = (i, ii, surveyOptionId) => {
        const {optionList} = this.store.questionDetailDta[i];
        optionList.forEach((vvv, iii) => {
            optionList[iii].isSelected = 2;
        });
        optionList[ii].isSelected = 1;
        console.log(JSON.parse(JSON.stringify(this.store.questionDetailDta)));
    };
    @action
    selectionfun = (i, ii, surveyOptionId) => {
        const optionItem = this.store.questionDetailDta[i].optionList[ii];
        optionItem.isSelected = +optionItem.isSelected === 1 ? 2 : 1;
    };
    @action
    ...
    Verificationfun = () => {
        ...
        return (this.store.questionDetailDta.length === Verificationval);
    };
    ...
    Submitfun = async (history) => {
        if (+this.type === 3) {
            ...
            if (this.Verificationfun()) {
                ...
                if (+result.resultCode === 0) {
                    setTimeout(function (){
                        history.push(`/PhasetwoMyQuestionnaireList/${type}`)
                    }, 1000);
                }
            } else {
                Toast.info(`内容不能为空`, 1);
