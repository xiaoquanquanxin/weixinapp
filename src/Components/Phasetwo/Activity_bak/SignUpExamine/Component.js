...
export default class PhasetwoActivitySignUpExamine extends React.Component {
    componentDidMount() {
        ...
        console.log(this.props, this.props.match.params.joinerId)
    ...
    render() {
        ...
        return <div className={"Components-Myactive-container"}>
            ...
            {findInfoByJoinerId.isOpenPic == 1 &&
                ...
                <img src={findInfoByJoinerId.picture} />
                    ...
                    <div className={"btnbox"}>
                {
                    ...
                    <div className={"btn"} onClick={() => { modifyfun(history) }}>
                        <MyButton type={"blue"} label={"修改信息"} width={(DetailsData.status == 2 &&DetailsData.activityStatus == 1)?"":163} />
                    </div>
                }
                {
                    DetailsData.status == 2 && DetailsData.activityStatus==0&&
                    <div className={"btn"} onClick={() => { scanQRCode(history, joinerId) }}>
                        <MyButton type={"blue"} label={"扫码签到"} width={DetailsData.modifyNumber == 0 && 163} />
                </div>
                }
                {DetailsData.status == 1?
                    <div className={"btn"} onClick={() => { cancelfun(history) }}>
                        <MyButton type={"white"} label={"取消报名"} width={DetailsData.modifyNumber==0&&163} />
                    </div>:""
                }
                {DetailsData.status == 3&&
                    <div className={"btn"} onClick={() => { history.push("/PhasetwoActivityList") }}>
