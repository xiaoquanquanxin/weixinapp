			...
			<div className={'Components-FamilyMembers-container'} >
				<div className={'Components-DemoLayout-container '} >
							...
							<div className={"userlist"}>
								{
									userList&&userList.map((subItem, i) => {
										...
										return(
										<div className={"user"} key={i} onClick={() => { Interestfun(subItem.wxUserId) }} >
									<div className="checkboxFive">
												<div className={`${Interestselects.includes(subItem.wxUserId) ? "li on" : 'li'}`} ></div>
									</div>
									<div className={"info"}>
												<div className={"name"}><span>{subItem.name}【{subItem.userType == 1 ? "业主":"家属"}】</span></div>
												<div className={"phone"}><span>性别：{subItem.sex == 1 ? "男" : "女"}</span><span>电话：{subItem.phoneNo}</span></div>
													{
								...
								getRoomInfo.length == 0?
									<div className={'addFamily grayColor'} 
										onClick={() => { history.push('/PhasetwoActivityUserInfo?auth=-1')}}>
										<p ><img src={addIcon} /></p >
										<p className={"adduser"}>添加报名成员</p>
									</div >
									:
									<div className={'addFamily grayColor'} 
										onClick={() => { history.push('/Certification/AddFamilyMembers') }}>
										<p ><img src={addIcon} /></p >
					...
					<div>
						...
						<div className={"fixed"}>
							<div className={"center"} onClick={() => { Interestselects.length > 0 ? submitfun(history) : "" }}>
