...
// import header2 from '../img/2.png';
// import Surplus from '../img/3.png';
/*自定义类*/
const tabs = [
    { title: "全部", sub: '0' },
    { title: "进行中", sub: '1' },
...
export default class PhasetwoActivityList extends React.Component {
    ...
    render() {
        ...
        const { actListdata, refreshing, tabval, height, actbottom } = storePhasetwoActivityList
        const { tabfun, onRefresh, signUpfun } = actionsPhasetwoActivityList
        let stext = ['','查看报名', '我要报名']
        let scolor = ['','#64C433','#888888']
        ...
        >
                ...
                <Tabs tabs={tabs}
                    initialPage={0}
                    onChange={(tab, index) => { tabfun(index); }}
                >
                </Tabs>
                <div className={"tab"}>
                    <div className={"list"}>
                                        ...
                                        <div onClick={() => {
                                            ...
                                            history.push('/PhasetwoActivityListDetail/' + v.activityId)
                                                ...
                                                <div className={"headerimg"}><img src={v.activityBanner} />
                                                    {v.activityBanner && <sup className="flag">{v.remainDay > 0 ? "只剩" + v.remainDay + "天" : "已结束"}</sup>}
                                                </div>
